...
public class AuthController {
    ...
    @PostMapping("/signup")
    public ApiResponse<RegisterUserResponse> createUserRegistration(@RequestBody RegisterUserRequest request) {
        return ApiResponse.of(SuccessStatus.SIGNUP_SUCCESS, authService.registerUser(request));
    }

    @Operation(summary = "로그인", description = "로그인 API입니다.")
    @PostMapping("/login")
    public ApiResponse<CreateLoginResponse> createUserLogin(@RequestBody CreateLoginRequest request) {
        return ApiResponse.of(SuccessStatus.LOGIN_SUCCESS, authService.authenticateUser(request));
    }

    @Operation(summary = "토큰 재발급", description = "토큰 재발급 API입니다.    \nRefresh Token을 통해 Access Token을 재발급합니다.")
    @PostMapping("/refresh")
    public ApiResponse<CreateTokenReissueResponse> createTokenReissue(@RequestBody CreateTokenReissueRequest request) {
    ...
    @PostMapping("/logout")
    public ApiResponse<Void> deleteUserLogout(
